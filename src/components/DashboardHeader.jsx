...
function DashboardHeader({ transactions }) {
  ...
  const { income, expense, balanceAmount } = amountCalculation({ transactions, categories });

  const headers = [
    { name: "Balance", amount: balanceAmount, textColor: "text-blue-500" },
    { name: "Income", amount: income, textColor: "text-emerald-500" },
    { name: "Expense", amount: expense, textColor: "text-red-500" },
  ];
  ...
  const renderedHeaders = headers.map((header) => {
    return (
      <Card
        key={header.name}
        className="bg-[#1f232c] flex-row items-center gap-10 max-w-xs !p-0 rounded-xl items-stretch"
      >
